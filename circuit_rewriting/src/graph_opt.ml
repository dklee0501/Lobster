...
let graph_performace : G.t -> vdmap -> int = fun graph -> fun vdmap ->
  ...
  let or_gate = or_size_of_graph graph in
  let xor_size = size_of_graph graph - mult_gate + (2 * or_gate) in
  ...
  let _ = print_endline("graph xor size : " ^ (string_of_int xor_size)) in
...
let graph_opt_by_case_file : G.t -> string -> string -> G.t = fun graph -> fun graph_filename -> fun case_filename ->
  ...
  *)
  ...
  let (_, _, _) = (opt_iter (graph, initial_vdmap, true) 0 []) in
