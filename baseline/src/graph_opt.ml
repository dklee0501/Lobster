...
let and_gate_exec_time = 
[
28;   57;   73;   156;  157; 
253;  278;  390;  435;  534; 
657;  759;  970;  1050; 1073;
1275; 1470; 1599; 1771; 1909;
2687; 2789; 2833; 3075; 3105; 
3277; 3323; 3534; 3581; 4375; 
4530; 4635; 4803; 5330; 5478; 
5626; 6473; 6587; 6722; 6954; 
7038; 7649; 7927; 8206; 10213
]
...
let graph_performance : G.t -> vdmap -> int = fun graph -> fun vdmap ->
  let depth = get_max_depth vdmap in
  let and_time = List.nth and_gate_exec_time (depth - 1) in
  let mult_gate = mult_size_of_graph graph in
  let _ = print_endline ("graph size : " ^ string_of_int (size_of_graph graph)) in
  let _ = print_endline("graph mult size : " ^ (string_of_int mult_gate)) in
  let _ = print_endline("and gate cost : " ^ string_of_int and_time) in
  let _ = print_endline("total cost : " ^ string_of_int (and_time * mult_gate)) in
  and_time * mult_gate 
...
let graph_opt_by_case_file : G.t -> string -> string -> G.t = fun graph -> fun graph_filename -> fun case_filename ->
  ...
  let _ = print_endline("initial mult size : " ^ (string_of_int mult_gate)) in
  ...
  let case_list = List.filter (fun (oldb, newb) -> get_bexp_size oldb < 40) case_list in
  let _ = print_endline ("reduced case num : " ^ string_of_int (List.length case_list)) in
  let l_list = List.map fst case_list in
  let case_size_list = List.map (fun x -> get_bexp_size x) l_list in
  let size_max = List.fold_left (fun acc -> fun size -> if(acc < size) then size else acc) 0 case_size_list in
  let size_min = List.fold_left (fun acc -> fun size -> if(size < acc) then size else acc) 100 case_size_list in
  let size_sum = List.fold_left (fun acc -> fun size -> acc + size) 0 case_size_list in
  let case_num = List.length case_size_list in
  let size_average = size_sum / case_num in
  let size_median = List.nth case_size_list (case_num / 2) in
  let _ = print_endline("max case size : " ^ (string_of_int size_max)) in
  let _ = print_endline("min case size : " ^ (string_of_int size_min)) in
  let _ = print_endline("average case size : " ^ (string_of_int size_average)) in
  let _ = print_endline("median case size : " ^ (string_of_int size_median)) in
  let _ = List.map (fun (oldb, newb) -> Pp.print_bexp oldb; print_string (" --> " ); Pp.print_bexp newb; print_newline() ) case_list in
  ...
  *)
  ...
  (*let critical_filename = graph_filename ^ "_critical_circuit" in*)
  (*let graph_cripath = find_oneof_critical_path graph initial_vdmap in*)
  (*let critical_graphstring = Eqn_printer.graph_to_critical_string graph graph_cripath in*)
  (*let _ = Eqn_printer.printf_string critical_filename critical_graphstring in*)
  ...
  *)
  let rec opt_iter : (G.t * vdmap * bool) -> int -> var list -> (G.t * vdmap * bool) = fun (acc_graph, acc_vdmap, acc_success) -> fun fail_num -> fun todo ->
    let old_depth = get_max_depth !optimal_vdmap in
    if (todo = []) then
      let _ = print_endline("graph opt iter finished") in
      let size = size_of_graph acc_graph in
      let _ = print_endline("============================================= graph opt iter once ===================================================") in 
      ...
      if(now_time -. start_time > 43200.) then
      ...
      if(new_depth < old_depth) then
        let old_cost = graph_performance !optimal_graph !optimal_vdmap in
        let new_cost = graph_performance new_graph new_vdmap in
        if(new_cost < old_cost) then
	  let _ = optimal_graph := new_graph; optimal_vdmap := new_vdmap in
  ...
  (*let ovlist = get_graph_ovlist opted_graph in
  let ilist = get_graph_ilist graph in
  let ilist_vertex = List.map (fun var -> (Node.VAR var, 0)) ilist in
  let essential = ((Node.CONST true, 0)::(Node.CONST false, 0)::ilist_vertex) @ ovlist in*)
  (*let opted_graph = cleanup_graph opted_graph essential in
  let _ = print_endline ("cleanup fin") in
  let opted_graph = redundancy_opt opted_graph in
  let _ = print_endline ("sharing detect fin") in
  let opted_graph = cleanup_graph opted_graph essential in
  let _ = print_endline ("cleanup fin") in*)
  (*let opted_graph_vdmap = get_var_depth_map opted_graph in*)
  (*let opted_graph_cripath = find_oneof_critical_path opted_graph opted_graph_vdmap in*)
  (*let opted_critical_graphstring = Eqn_printer.graph_to_critical_string opted_graph opted_graph_cripath in*)
  let opted_result_filename = graph_filename ^ "_opted_result" in
  (*let opted_result_critical_filename = graph_filename ^ "_opted_critical_result" in*)
  let _ = Eqn_printer.printf_graph opted_result_filename opted_graph in
  (*let _ = Eqn_printer.printf_string opted_result_critical_filename opted_critical_graphstring in*)
  opted_graph
