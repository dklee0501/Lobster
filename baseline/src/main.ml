...
let filename = let regexp = Str.regexp_string "../baseline/epfl_bench/" in Str.global_replace regexp "" filename
let filename = let regexp = Str.regexp_string "paper_bench/" in Str.global_replace regexp "" filename
let filename = let regexp = Str.regexp_string "epfl_bench/" in Str.global_replace regexp "" filename
let filename_length = String.length filename

let filename_length = String.length filename
let time_length = String.length time_string

let old_depth_string = (string_of_int old_depth)
let old_depth_len = String.length old_depth_string
let new_depth_string = (string_of_int new_depth)
let new_depth_len = String.length new_depth_string

let rec empty_n_string n = if(n = 0) then "" else " " ^ empty_n_string (n-1)

let _ = prerr_string(filename ^ (empty_n_string (16 - filename_length)) ^ empty_n_string (5 - old_depth_len) ^ (old_depth_string) ^ "          " ^ empty_n_string (5 - new_depth_len) ^ (new_depth_string) ^ empty_n_string (17 - time_length) ^ time_string)
