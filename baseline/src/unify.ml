...
let rec is_equal_cbexp : cbexp -> cbexp -> bool = fun b1 -> fun b2 ->
  ...
  | (AND (a, b), AND(c, d)) 
  | (XOR (a, b), XOR(c, d)) 
  | (OR (a, b), OR(c, d)) -> ((is_equal_cbexp a c) && (is_equal_cbexp b d)) || ((is_equal_cbexp a d) && (is_equal_cbexp b c))
...
let unify : G.t -> cbexp -> G.vertex -> (subst * bool * bexp) = fun graph -> fun old -> fun tgt_node ->
  ...
  let rec unify_helper : (subst * bool) -> cbexp -> G.vertex -> (subst * bool * bexp) = fun (subst, current_state) -> fun old -> fun tgt_node ->
    if(not current_state) then
      ...
      | (VAR x, Node.CONST b) -> ((x, CONST b)::subst, true, CONST b)
      | (VAR x1, Node.VAR x2) -> 
        ((x1, CVAR x2)::subst, true, VAR x2)
      | (VAR x, _) -> 
        let tgt_bexp = get_bexp_of_node graph tgt_node in
        let _ = print_endline("unify tgt_bexp : " ^ Pp.bexp_to_string tgt_bexp) in
	let tgt_cbexp = bexp2cbexp tgt_bexp in
	((x, tgt_cbexp)::subst, true, tgt_bexp)
      | (AND (b1, b2), Node.AND)  
      | (OR (b1, b2), Node.OR)  
      | (XOR (b1, b2), Node.XOR) -> 
